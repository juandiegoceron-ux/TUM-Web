...
function Dashboard() {
    ...
    const gameCards: GameCard[] = [
        { id: 0, title: 'Próximamente', active: false, lock: true },
        { id: 1, title: 'Guía a T.U.M', active: true, icon: Compass },
        { id: 2, title: 'Próximamente', active: false, lock: true },
        { id: 3, title: 'Próximamente', active: false, lock: true },
        { id: 4, title: 'Próximamente', active: false, lock: true },
        { id: 5, title: 'Próximamente', active: false, lock: true },
        { id: 6, title: 'Próximamente', active: false, lock: true },
        { id: 7, title: 'Próximamente', active: false, lock: true },
        { id: 8, title: 'Próximamente', active: false, lock: true },
        { id: 9, title: 'Próximamente', active: false, lock: true },
    ...
    const { nombre, email, logout, role } = useAuth();
            ...
            <div className="dashboard-container">
                ...
                <div className="dashboard-banner">
                    <div className="banner-left">
                        <div className="user-avatar">{nombre?.charAt(0).toUpperCase() || 'E'}</div>
                        <div className="user-greeting">
                            <span className="greeting-label">¡Hola, explorador!</span>
                            <h3 className="greeting-name">Bienvenido, {nombre || 'Explorador'}</h3>
                        </div>
                    </div>
                        ...
                        <div className="tab-pane-content animate-fade-in">
                            <div className="profile-section">
                                <div className="profile-header">
                                    <div className="profile-avatar-large">{nombre?.charAt(0).toUpperCase() || 'E'}</div>
                                    <h2>{nombre || 'Explorador'}</h2>
                                    <p>{email}</p>
                                </div>
                ...
                <div className="dashboard-navigation">
                    <button className={`nav-btn ${activeTab === 'inicio' ? 'active-nav' : ''}`} onClick={() => setActiveTab('inicio')}>
                        <Home size={18} />
                        <span>Inicio</span>
                    </button>

                    <button className={`nav-btn ${activeTab === 'conoce' ? 'active-nav' : ''}`} onClick={() => setActiveTab('conoce')}>
                        <HelpCircle size={18} />
                        <span>Conoce a T.U.M.</span>
                    </button>

                    <button className={`nav-btn ${activeTab === 'perfil' ? 'active-nav' : ''}`} onClick={() => setActiveTab('perfil')}>
                        <User size={18} />
                        <span>Perfil</span>
                    </button>

                    <button className={`nav-btn ${activeTab === 'ajustes' ? 'active-nav' : ''}`} onClick={() => setActiveTab('ajustes')}>
                        <Settings size={18} />
                        <span>Ajustes</span>
                    </button>
                </div>
